...
const BlogPost = () => {
    ...
    const fetchBlog = async () => {
      try {
        ...
        if (data.success) {
          ...
          // Assuming basic sanitization / summary generation for description
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      await fetch(`http://localhost:5000/api/blogs/${blog.id}/like`, { method: "POST" });
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const submitComment = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/blogs/${blog.id}/comments`, {
        method: "POST",
    ...
    <Layout>
      <article className="pt-32 pb-16 bg-background min-h-screen">
        <div className="container mx-auto max-w-4xl px-4">
          <Link to="/" className="inline-flex items-center text-muted-foreground hover:text-primary mb-8 font-medium">
          ...
          <div className="flex flex-wrap items-center justify-between border-b border-border/50 pb-8 mb-10 gap-4">
            <div className="text-muted-foreground font-medium text-lg">
              Published on {new Date(blog.published_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}
            </div>
